...
var respond = function (handler) {
  return function (req, res, next) {
    ...
    var done = function () {
      // 如果session中有_wait标记
      if ((message.MsgType === 'text' || message.MsgType === 'event')&& req.wxsession && req.wxsession._wait) {
        var list = List.get(req.wxsession._wait);
        if(message.MsgType ==='event'){
          var handle = list.get(message.EventKey);
        }else{
...
Handler.prototype.getHandler = function (type) {
  return  this.handlers[type] || this.handle ||function (info, req, res, next) {
    next();
  };
};
